...
class InteractiveLabelingTool:
    def __init__(self, image_path, predictions, parent_window=None, text_display=None):
        ...
        self.boxes = predictions.tolist() if isinstance(
        ...
        self.canvas = tk.Canvas(
            main_frame, width=self.photo.width(), height=self.photo.height())
        self.canvas.pack(side=tk.TOP)

        # Create text display if not provided
        self.text_display = text_display if text_display else tk.Text(
        ...
        # Remove box button
        remove_box_button = tk.Button(
            button_frame, text="Remove Box", command=self.remove_box)
        remove_box_button.pack(side=tk.LEFT, padx=5, pady=5)

        # Remove all boxes button
        remove_all_boxes_button = tk.Button(
            button_frame, text="Remove All Boxes", command=self.remove_all_boxes)
        remove_all_boxes_button.pack(side=tk.LEFT, padx=5, pady=5)

        # Save annotations button
        save_button = tk.Button(
            button_frame, text="Save Annotations", command=self.save_annotations)
        save_button.pack(side=tk.LEFT, padx=5, pady=5)

        # Detect Text button
        detect_text_button = tk.Button(
            button_frame, text="Detect Text", command=self.detect_text_in_boxes)
    ...
    def detect_text_in_boxes(self):
        ...
        save_dir = os.path.join(r"/home/omen/Documents/Shiv/dataset/cropped",
                                os.path.splitext(os.path.basename(self.image_path))[0])
        os.makedirs(save_dir, exist_ok=True)
        
        
        original_image_path = os.path.join('/home/omen/Documents/Shiv', "original_image.png")
        ...
        cord_dir = '/home/omen/Documents/Shiv'
        json_path = os.path.join(cord_dir, "bounding_boxes.json")
        box_data = []
        for i, box in enumerate(self.boxes):
        ...
        with open(json_path, 'w') as json_file:
            json.dump(box_data, json_file, indent=4)
...
def save_yolo_annotations(image_path, boxes):
    ...
    labels_dir = r"/home/omen/Documents/Shiv/dataset/labels/train"
...
def detect_text(cropped_region):
    # Your text detection logic here
    return ["Some detected text"]
...
def crop_and_save_bounding_boxes(image, results, save_dir="save"):
    ...
    if isinstance(results, list) and results and 'boxes' in results[0]:
        ...
        print("this is result if it is list of dictionaries with 'boxes' key", results)
        print("Boxes",boxes)
    # If results is a numpy array of boxes
    elif isinstance(results, np.ndarray):
        boxes = results
        print("this is result if it is results is a numpy array of boxes", results)
        print("Boxes",boxes)

    else:
        print("Unexpected results format")
        print("this is result if it is results is a Unexpected results format", results)
    ...
    for seq_num, box_info in enumerate(sorted_boxes, 1):
        ...
        #     print(f"Invalid bounding box: x1={x1}, y1={y1}, x2={x2}, y2={y2}")
        #     continue
        ...
        #     print("Cropped region is empty, skipping save.")
        #     continue  # Ensure it's an empty list, not None

        # else:
        #     # Detect text in the cropped region
        #     detected_texts = detect_text(cropped_region)

        # # Safely iterate over detected texts
    # Inside your cropping and saving logic
        save_path = os.path.join(save_dir, f"text_region00_{seq_num:03d}.png")

        if cropped_region is None and cropped_region.size == 0:
            print(f"Cropped region is empty for bounding box {x1, y1, x2, y2}. Skipping.")
            # cv2.imshow(f"Cropped Region is empty for bounding box  {seq_num}", cropped_region)
            # cv2.waitKey(0)  # Wait for a key press to view each region
            # cv2.destroyAllWindows()
            continue
        
        if cropped_region is None:
            print(f"Cropped region is None {x1, y1, x2, y2}. Skipping.")
            continue
        
        if cropped_region.size == 0:
            print(f"Cropped region is cropped_region.size == 0  for bounding box {x1, y1, x2, y2}. Skipping.")
            # cv2.imshow(f"Cropped Region is empty for bounding box  {seq_num}", cropped_region)
            # cv2.waitKey(0)  # Wait for a key press to view each region
            # cv2.destroyAllWindows()
            continue
        # else:
        #     cv2.imshow(f"Cropped Region {seq_num}", cropped_region)
        # cv2.waitKey(0)  # Wait for a key press to view each region
        # cv2.destroyAllWindows()
        try:
            cv2.imwrite(save_path, cropped_region)
        except cv2.error as e:
            print(f"Failed to save cropped region to {save_path}: {e}")
...
def iterative_pipeline(input_path, model_path, iterations=5):
    ...
    data_yaml_path = r"/home/omen/Documents/Shiv/dataset/data.yaml"
...
names: ['word']""")
    ...
    for i in range(iterations):
        ...
        for img_path in images:
            ...
            save_dir = os.path.join(r"/home/omen/Documents/Shiv/dataset/cropped", os.path.splitext(
...
input_path = r"/home/omen/Documents/Shiv/dataset/images/train"
model_path = "best.pt"
iterations = 1
